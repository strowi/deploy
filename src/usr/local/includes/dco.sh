...
install_dco() {
  ...
  for HOST in ${SSH_REMOTE}; do
    echo -en "## installing docker-compose on: ${SSH_USERNAME}@${HOST}     "
...
docker_prune() {
  for HOST in ${SSH_REMOTE}; do
    echo -en "## pruning docker-system on: ${SSH_USERNAME}@${HOST}     "
...
deploy_dco() {
  for HOST in ${SSH_REMOTE}; do
    ...
    if [[ -z "${DRY_RUN}" ]]; then
      ...
      ${SSH_CMD} "${SSH_USERNAME}@${HOST}" "cd ${SSH_REMOTE_DST} && set -ae && /home/${SSH_USERNAME}/.local/bin/docker-compose ${DOCKER_OPTS_PRE} pull --include-deps -q && /home/${SSH_USERNAME}/.local/bin/docker-compose ${DOCKER_OPTS_PRE} up -d --remove-orphans --no-build ${DOCKER_OPTS_TRAILING}"
      ...
      echo "ck_deployment{environment_url=\"${CI_ENVIRONMENT_URL}\", environment=\"${CI_ENVIRONMENT_SLUG}\", pipeline=\"${CI_PIPELINE_URL}\"} $(date +%s)" >/tmp/metrics
      echo -e "######################################################################################################"
    else
